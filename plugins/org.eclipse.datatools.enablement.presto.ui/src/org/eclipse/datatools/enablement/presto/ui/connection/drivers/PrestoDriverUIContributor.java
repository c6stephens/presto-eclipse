...
import org.eclipse.datatools.enablement.presto.ui.Messages;
...
public class PrestoDriverUIContributor implements IDriverUIContributor,
	...
	private static final String CUI_NEWCW_GENERAL_TAB_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.general.tab"); //$NON-NLS-1$

	private static final String CUI_NEWCW_OPTIONAL_TAB_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.optional.tab"); //$NON-NLS-1$

	private static String CUI_NEWCW_DATABASE_LBL_UI_ = Messages
	.getString("PrestoProfileDetailsWizardPage.databaseName.label"); //$NON-NLS-1$
	
	private static final String CUI_NEWCW_DBLOCATION_LBL_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.0"); //$NON-NLS-1$

	private static final String CUI_NEWCW_DBBROWSE_BTN_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.1"); //$NON-NLS-1$

	private static final String CUI_NEWCW_USERNAME_LBL_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.userName.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_PASSWORD_LBL_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.password.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_SAVE_PASSWORD_LBL_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.persistpassword.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_CONNECTIONURL_LBL_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.url.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_OPTIONAL_PROPERTIES_LBL_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.optionalProps.label"); //$NON-NLS-1$	

	private static final String CUI_NEWCW_DRIVER_NAME_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.driverName.label"); //$NON-NLS-1$
	
	private static final String CUI_NEWCW_DATABASE_SUMMARY_DATA_TEXT_ = Messages
	.getString("PrestoProfileDetailsWizardPage.summary.database.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_DATABASE_LOCATION_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.databaselocation.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_USERNAME_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.userName.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_URL_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.url.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_SAVE_PASSWORD_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.persistpassword.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_OPTIONAL_CONNECTION_PROPERTIES_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.connProps.label"); //$NON-NLS-1$

	private static final String CUI_NEWCW_TRUE_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.true"); //$NON-NLS-1$

	private static final String CUI_NEWCW_FALSE_SUMMARY_DATA_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.summary.false"); //$NON-NLS-1$

	private static final String CUI_NEWCW_FILE_BROWSER_TITLE_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.FileDialog.title.filebrowse"); //$NON-NLS-1$

	private static final String CUI_NEWCW_FILE_BROWSER_MESSAGE_TEXT_ = Messages
			.getString("PrestoProfileDetailsWizardPage.FileDialog.msg.filebrowse"); //$NON-NLS-1$

	private static final String CUI_NEWCW_VALIDATE_DATABASE_REQ_UI_ = Messages
	.getString("PrestoProfileDetailsWizardPage.database.validation.message"); //$NON-NLS-1$

	private static final String CUI_NEWCW_VALIDATE_DBLOCATION_REQ_UI_ = Messages
			.getString("PrestoProfileDetailsWizardPage.databaselocation.validation.message"); //$NON-NLS-1$
		...
		if (parentComposite == null || parentComposite.isDisposed() || (this.isReadOnly != isReadOnly)) {
			...
			browseDatabaseLocation.addListener(SWT.Selection, new Listener() {
				...
				public void handleEvent(Event event) {
					PrestoDriverUIContributor.this.browseForDBFolder();
				}
	...
	public void loadProperties() {
		removeListeners();
		PrestoJDBCURL url = new PrestoJDBCURL(this.properties
	...
	protected void updateURL() {
		String url = "jdbc:presto:" + databaseLocationText.getText(); //$NON-NLS-1$
		urlText.setText(url);
	}

	protected class PrestoJDBCURL {
		 ...
		 * @param url
		 */
		public PrestoJDBCURL(String url) {
			parseURL(url);
		}
